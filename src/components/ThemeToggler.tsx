...
export default function ThemeToggler(){
  ...
  return (
    <div className="dropdown dropdown-end">
      ...
      <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52 max-h-72 overflow-auto flex flex-col space-y-1">
        {THEMES.map(t => (
          <li key={t} className="w-full">
            <button type="button" className={`w-full text-left px-2 py-1 rounded ${t===currentTheme? 'font-semibold bg-base-200':''}`} onMouseDown={(e)=>{e.preventDefault(); setCurrentTheme(t)}}>
              {t}
            </button>
          </li>
        ))}
